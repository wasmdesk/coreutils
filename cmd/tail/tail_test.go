...
func env(t *testing.T, args ...string) (*fsx.Env, *bytes.Buffer, *bytes.Buffer, *fsx.MemFS) {
	...
	return &fsx.Env{Args: append([]string{"tail"}, args...), Stdout: &out, Stderr: &errb, FS: m, Cwd: "/"}, &out, &errb, m
}

func TestTailDefault10(t *testing.T) {
	e, out, _, m := env(t, "/f")
	body := ""
	for i := 1; i <= 15; i++ {
		body += "L\n"
	...
	if strings.Count(out.String(), "\n") != 10 {
		t.Errorf("got %d lines, want 10", strings.Count(out.String(), "\n"))
	}
}
...
func TestTailNoTrailingNL(t *testing.T) {
	e, out, _, m := env(t, "-n", "2", "/f")
	_ = m.WriteFile("/f", []byte("a\nb\nc"))
	if rc := Run(e); rc != exit.Ok {
		t.Fatalf("rc = %d", rc)
...
func TestTailNoArgs(t *testing.T) {
	e, _, errb, _ := env(t)
...
func TestTailEmptyArgv(t *testing.T) {
...
func TestTailDashNNoValue(t *testing.T) {
	e, _, errb, _ := env(t, "-n")
	if rc := Run(e); rc != exit.Usage {
		t.Errorf("rc = %d", rc)
	}
...
func TestTailDashNBadValue(t *testing.T) {
...
func TestTailPrettyErr(t *testing.T) {
	cases := []struct {
		err  error
		want string
	}{
